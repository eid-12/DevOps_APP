...
export class ProjectService {
  ...
  create(payload: CreateProjectRequest): Observable<Project> {
    ...
    try {
      ...
      if (user.accountStatus === 'SUSPENDED') {
        return throwError(() => ({ error: { message: 'Suspended users cannot create projects' } })).pipe(delay(200));
      }
  ...
  addService(projectId: string, payload: CreateServiceRequest): Observable<Service> {
    ...
    try {
      const user = this.auth.requireUser();
      if (!user.deploymentEnabled || user.accountStatus === 'SUSPENDED') {
        return throwError(() => ({ error: { message: 'Deployment access disabled' } })).pipe(delay(200));
      }
