...
|--------|------|--------|
| POST | `/api/auth/login` | |
| POST | `/api/auth/register` | First code takes a slot; **5 / hour** site-wide |
| POST | `/api/auth/forgot-password` | 503 if mail off; generic 200 if on; 30 min link; same 5/hour cap |
| POST | `/api/auth/reset-password` | Password-reset JWT |
| POST | `/api/auth/verify-email` | 6-digit code, 15 min TTL |
| POST | `/api/auth/resend-verification` | 60 s cooldown; **5 / hour** site-wide |
